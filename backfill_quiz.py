...
# Supabase에서 전체 퀴즈 가져오기
res = requests.get(
    f'{SUPABASE_URL}/rest/v1/Quiz?select=*&order=date.asc',
    headers=headers,
    params={'limit': 2000}
)
all_quizzes = res.json()
print(f"전체 퀴즈: {len(all_quizzes)}개")
...
def github_push(path, content, message):
    ...
    if sha:
        data['sha'] = sha
    res = requests.put(url, headers=gh_headers, json=data)
    return res.status_code in [200, 201]

def make_html(date, quizzes):
    quizzes_json = json.dumps(quizzes, ensure_ascii=False).replace('<', '\\u003c').replace('>', '\\u003e')
    return f"""<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>{date} 금융 퀴즈 10문제 - 매십문</title>
<meta name="description" content="{date} 매십문 금융 퀴즈 10문제. 매일 업데이트되는 금융/경제 퀴즈로 금융 자격증을 준비하세요.">
...
print(f"생성할 날짜: {dates}")

for i, date in enumerate(dates):
    quizzes = by_date[date]
    html = make_html(date, quizzes)
    result = github_push(f'quiz/{date}.html', html, f'Backfill quiz {date}')
    print(f"[{i+1}/{len(dates)}] {date}: {'성공' if result else '실패'}")
